    ...
    (
      let
        tigrc = ''
          ...
          bind generic . !@sh -c "echo -n %(commit) | ${xsel}/bin/xsel --clipboard --input"
          bind generic ; none
          bind generic ;s :!git status
          bind generic ;f :!git fetch --all --prune
          bind generic ;yy :!git stash
          bind generic ;yp :!git stash pop
          bind generic ;p :!git push
          bind generic ;P :!?git push -f
          bind generic ;j :!git rebase --abort
          bind generic ;k :!git rebase --continue
          bind main    ;i ?git rebase -i %(commit)
          bind main    ;r ?git rebase %(branch)
          bind main    ;o ?git reset --soft %(commit)
